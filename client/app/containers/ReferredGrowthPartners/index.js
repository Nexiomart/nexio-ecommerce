...
class ReferredGrowthPartners extends React.PureComponent {
  state = {
    isLoading: true,
    partners: [],
    error: null
  ...
  async componentDidMount() {
    try {
      const res = await axios.get(`${API_URL}/growthPartner/referred`);
  ...
  render() {
    ...
    return (
      <div className="referred-growth-partners">
        <SubPage title={"Referred Growth Partners"} isMenuOpen={null}>
          ...
          {!isLoading && !error && partners.length > 0 && (
            <>
              ...
              {/* Read-only usage of GrowthPartnerList (no action handlers passed) */}
...
export default connect(null, actions)(ReferredGrowthPartners);
